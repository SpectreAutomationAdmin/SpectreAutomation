// Local functional integration visual check. Renders /preview/
// schedule-functional (which mounts the REAL production ScheduleView
// with concept-shaped props) at 1440x900 and builds a normal
// full-color side-by-side vs the authoritative approved reference.
...
const REFERENCE  = path.join(OUT, "functional-reference.png");
const RENDER     = path.join(OUT, "functional-render.png");
const SIDEBYSIDE = path.join(OUT, "functional-side-by-side.png");
const LOCAL = process.env.PREVIEW_URL ?? "http://localhost:3000/preview/schedule-functional";

test.use({ viewport: { width: 1440, height: 900 } });

test("functional integration: real ScheduleView renders approved shell", async ({ browser, page }) => {
...
<div style="display:flex;gap:12px;padding:12px;background:#111;">
  ...
  <div style="flex:0 0 auto;background:#fff;">
    <div style="padding:6px 10px;background:#efe;color:#123;font:600 12px system-ui;">FUNCTIONAL RENDER 1440x900 (real ScheduleView)</div>
