...
//
// Rev-4: the Spectre chrome is now the DEFAULT for every admin
...
//
// Everything else — Members, AP Vendors, Vendor Timeline, AP
// Invoices, Capture Inbox, Approvals, Governance, Analytics, etc. —
// now renders on the same `SpectreShell` as Mission Control, so the
// sidebar identity, navigation, and chrome remain stable while page
// content changes.
//
// The prior opt-in constants are removed. If a future genuine
// exception is needed, add it as a `LEGACY_CHROME_PREFIXES` opt-OUT
// list here with a documented reason.
