...
export default function DesktopQuickLinksCard({ items }: Props) {
  ...
  return (
    <section
      className="rounded-2xl bg-club-cream border border-stone-200/60 p-6"
    ...
    >
      <header className="flex items-center gap-3 pb-4">
        ...
        <h2 className="font-serif text-[19px] text-club-ink">Quick Links</h2>
      </header>
      <ul className="rounded-xl bg-white border border-stone-200/70 divide-y divide-stone-200/70">
        {list.map((it) => {
          const inner = (
            <div className="flex items-center px-4 py-3.5">
              <span className={`flex-1 text-[14.5px] ${it.href ? "text-club-ink" : "text-stone-500"}`}>{it.label}</span>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" className="text-stone-400 shrink-0 ml-3">
                <path d="M14 3h7v7" />
                <path d="M10 14 21 3" />
