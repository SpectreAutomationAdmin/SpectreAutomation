...
export default async function MemberProfilePage({ params, searchParams }: PageProps) {
  ...
  const member = await prisma.member.findFirst({
    ...
    include: {
      ...
      // subtle indicator renders in the profile header and (for
      // callers with `hr:directory:view`) links to the Employee
      // profile. No HR data is duplicated into the Member profile —
      // this is purely a navigation affordance.
      employee: { select: { id: true, employeeNumber: true, employeeLifecycle: true } },
    },
  });
  if (!member) notFound();

  // HR-2A — permission-gated Employee link. Callers without
  // `hr:directory:view` see plain text (no leak, no link).
        ...
        employeeLink={
          member.employee
            ? {
                ...
                employeeNumber: member.employee.employeeNumber,
                canNavigate: canSeeEmployeeLink,
              }
            : null
        }
      />
